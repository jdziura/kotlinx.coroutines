...
abstract class SchedulerTestBase : TestBase() {
    companion object {
        ...
        private fun maxSequenceNumber(): Int? {
            return Thread.getAllStackTraces().keys.asSequence().filter { it is CoroutineScheduler.Worker || it is DotnetBasedCoroutineScheduler.Worker || it is GoBasedCoroutineScheduler.Worker }
